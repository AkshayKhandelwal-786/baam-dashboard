...
const DefaultPalette = (mode: Palette['mode'], skin: Skin, themeColor: ThemeColor): Palette => {
  ...
  return {
    customColors: {
      ...
      bodyBg: mode === 'light' ? '#F4F5FA' : '#151418', // Same as palette.background.default but doesn't consider bordered skin
      trackBg: mode === 'light' ? '#F0F2F8' : '#474360',
      avatarBg: mode === 'light' ? '#F0EFF0' : '#3F3B59',
      darkBg: skin === 'bordered' ? '#151418' : '#151418',
      lightBg: skin === 'bordered' ? whiteColor : '#F4F5FA',
      tableHeaderBg: mode === 'light' ? '#F9FAFC' : '#2b2b2b'
    },
    mode: mode,
    common: {
      black: '#000',
    ...
    background: {
      paper: mode === 'light' ? whiteColor : '#151418',
