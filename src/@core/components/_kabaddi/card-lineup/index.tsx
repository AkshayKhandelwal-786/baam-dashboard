...
const CardLineup = (props: CardSnippetProps) => {
  ...
  const [playing7s, setPlaying7s] = useState<Array<number>>([])
  ...
  const handlePlaying7Select = (playerId: number) => {
    const selected = playing7s.includes(playerId)

    setPlaying7s(prev =>
      selected ? [...new Set(playing7s.filter(f => f != playerId))] : [...new Set([...prev, playerId])]
    )
    if (selected) {
          ...
          <ListSubheader sx={{ padding: 'unset' }}>
            <ListItemButton>
              ...
              <ListItemSecondaryAction tabIndex={10} style={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography variant='body2'>Playing 7</Typography>
                ...
                <ListItem disablePadding>
                  <ListItemButton
                    onClick={(e: any) => {
                      ...
                      handlePlaying7Select(player.id)
                    }}
                  >
                    <ListItemAvatar>
                      <Avatar
                    ...
                    <ListItemSecondaryAction tabIndex={10}>
                      {/* <Checkbox
                        ...
                        onChange={() => handlePlaying7Select(player.id)}
                        checked={playing7s.includes(player.id)}
                      /> */}
                      ...
                      <IconButton
                        ...
                        color={playing7s.includes(player.id) ? 'primary' : 'default'}
                        sx={{ opacity: playing7s.includes(player.id) ? 1 : 0.2 }}
                      >
                      ...
                      <IconButton
                        ...
                        sx={{ opacity: substitutes.includes(player.id) ? 1 : playing7s.includes(player.id) ? 0.2 : 0 }}
                      >
