...
const CardPreSquad = (props: CardSnippetProps) => {
  ...
  const { KBD_ROLES } = useConstantStore()
  ...
  const handlePlaying7Select = (playerId: number, type: 'ROLE' | 'PRICE', value: any) => {
                    ...
                    >
                      ...
                      {KBD_ROLES.map(role => {
                        return (
                          <IconButton
                            onClick={e => {
                              handlePlaying7Select(player?.player_id, 'ROLE', role)
                            }}
                            size='medium'
                            color={player?.role == role ? 'primary' : 'default'}
                            sx={{ opacity: player?.role == role ? 1 : 0.7, height: 80, width: 80 }}
                          >
                      ...
                      <TextField
                        ...
                        value={player?.price}
                        onChange={e => {
                          handlePlaying7Select(player?.player_id, 'PRICE', parseInt(e.target.value))
                        }}
                        type={'number'}
                      />
                    </ListItemSecondaryAction>
