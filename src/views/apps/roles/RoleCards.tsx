...
import useRoleStore, { Abilities, Ability, Permission } from 'src/features/roles/role.service'
...
const RolesCards = ({ defaultSchemaValues, describedSchema, open, setOpen, schema }: Props) => {
  ...
  useEffect(() => {
    ...
    if (store?.role?.detail?.permissions) {
      ...
      setPermission(store?.role?.detail?.permissions as any)
    }
  }, [store?.role?.detail])
  ...
  const handlePermission = (moduleId: string, ability?: Abilities, ids?: number[]) => {
    const value = getValues()

    setPermission(prev => {
      const newPermission = { ...prev }

      if (!newPermission[moduleId]) {
        newPermission[moduleId] = {
          ability: [],
          ids: []
        }
      }
      if (ability) {
        if (newPermission[moduleId].ability.includes(ability)) {
          newPermission[moduleId].ability = newPermission[moduleId].ability.filter(f => f != ability).sort()
        } else {
          newPermission[moduleId].ability = [...newPermission[moduleId].ability, ability].sort()
        }
      }

      if (ids?.length) {
        newPermission[moduleId].ids = ids
      }
  ...
  const handleSelectAllPermission = () => {
    ...
    setPermission(prev => ({ ...prev, ...selected }))
    setAllPermissions(prev => !prev)
  }
              ...
              <TableBody>
                {module?.list?.length ? (
                  module?.list.map(item => {
                    ...
                    return (
                      <TableRow key={item?._id} sx={{ '& .MuiTableCell-root:first-of-type': { pl: '0 !important' } }}>
                        <TableCell
                          sx={{
                            fontWeight: 600,
                            whiteSpace: 'nowrap',
                        ...
                        {abilities.map(ability => {
                          return (
                            <TableCell>
                              <FormControlLabel
                                label={ability.name}
                                checked={entry?.[item._id]?.ability?.includes(ability._id)}
                                control={
                                  <Checkbox
                                    size='small'
                                    onChange={e => {
                                      handlePermission(item._id, ability._id)
                                    }}
                                  />
                                }
                              />
