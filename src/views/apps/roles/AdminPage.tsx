...
const defaultColumns: PlanListColumn[] = [
  ...
  //   renderCell: ({ row }: CellType) => (
  //     <Typography variant='body2'>{format(new Date(row?.createdAt), 'dd MMM yyyy hh:mm:ss')}</Typography>
  //   )
  // },
  // {
  //   flex: 0.1,
  ...
  //   renderCell: ({ row }: CellType) => (
  //     <Typography variant='body2'>{format(new Date(row?.updatedAt), 'dd MMM yyyy hh:mm:ss')}</Typography>
  //   )
  // }
]
...
const schema = yup.object().shape({
  ...
  role: yup.string().label('Role').meta({ type: 'select' }).required()
})
...
const AdminPage = () => {
  ...
  const schemas = {
    ROLE: {
      ...
      //   .number()
      //   .label('Game')
      //   .meta({ type: 'select', values: gameStore.game.list.map(item => ({ label: item?.name, value: item?.id })) })
      //   .required()
    }
  }
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <IconButton
              ...
              onClick={() => {
                for (let key in defaultValues) {
                  setValue(key as any, typeof row[key] == 'object' ? row[key]?._id : row[key])
                  handleEditClickOpen()
                }
          ...
          <Tooltip title={`Add New Permission`}>
            <IconButton
              size='small'
              onClick={() => {
                roleStore.get.detail(row?._id, row, true)
                handleAdd()
              }}
            >
          ...
          <Card>
            ...
            <DataGrid
              ...
              onPaginationModelChange={({ page, pageSize }) => {
                if (page == store.admin.page && pageSize == store.admin.size) return
                ...
                <Grid container spacing={6}>
                  {Object.keys(describedSchema.fields).map(fieldName => {
                    ...
                    if (field.meta?.type == 'select') {
                      if (fieldName == 'role') {
                        field.oneOf = roleStore.role.list.map(role => ({ value: role._id, label: role.name }))
                      }
                    }
